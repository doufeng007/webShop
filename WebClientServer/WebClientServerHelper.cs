...
namespace WebClientServer
{
    ...
    public class HttpClientHelper
    {
        ...
        public static string GetResponseForIM(string url, out bool isSuccess, string header_token = null, bool isRetry = false)
        {
            HttpClient httpClient = new HttpClient(new HttpClientHandler() { AutomaticDecompression = DecompressionMethods.GZip });
            HttpResponseMessage response = null;
            isSuccess = false;
            try
            {
                ...
                httpClient.DefaultRequestHeaders.Accept.Add(
                   new MediaTypeWithQualityHeaderValue("application/json"));
                if (!string.IsNullOrWhiteSpace(header_token))
                    httpClient.DefaultRequestHeaders.Add("Authorization", $"Bearer {header_token}");
                Task<HttpResponseMessage> taskResponse = httpClient.GetAsync(url);
                taskResponse.Wait();
                response = taskResponse.Result;
                Task<System.IO.Stream> taskStream = response.Content.ReadAsStreamAsync();
                ...
                else if (response.StatusCode == HttpStatusCode.RequestTimeout)
                {
                    if (!isRetry)
                    {
                        return GetResponseForIM(url, out isSuccess, header_token, true);
                    }
                    else
                    {
                        Abp.Logging.LogHelper.Logger.Error($"调用IM_api报错：url:{url},状态码：{response?.StatusCode ?? 0},返回内容:{result}");
                        return result;
                    }
                }
                else
            ...
            {
                Abp.Logging.LogHelper.Logger.Error($"调用IM_api报错：url:{url},异常：{exception.Message}");
                return null;
            }
            finally
            {
        ...
        public static string PostResponseForIM(string url, object parameterobj, out bool isSuccess, string header_token = null, bool isRetry = false)
        {
            HttpClient httpClient = new HttpClient(new HttpClientHandler() { AutomaticDecompression = DecompressionMethods.GZip });
            HttpResponseMessage response = null;
            string result = "";
            ...
            try
            {
                ...
                httpClient.CancelPendingRequests();
                httpClient.DefaultRequestHeaders.Clear();
                string postData = JsonConvert.SerializeObject(parameterobj);
                HttpContent httpContent = new StringContent(postData);
                httpContent.Headers.ContentType = new MediaTypeHeaderValue("application/json");
                if (!string.IsNullOrWhiteSpace(header_token))
                    httpClient.DefaultRequestHeaders.Add("Authorization", $"Bearer {header_token}");
                ...
                else if (response.StatusCode == HttpStatusCode.RequestTimeout)
                {
                    if (!isRetry)
                    {
                        return PostResponseForIM(url, parameterobj, out isSuccess, header_token, true);
                    }
                    else
                    {
                        Abp.Logging.LogHelper.Logger.Error($"调用IM_api报错：url:{url},参数：{parameterobj},状态码：{response?.StatusCode ?? 0},返回内容:{result}");
                        return result;
                    }
                }
                else
            ...
            {
                Abp.Logging.LogHelper.Logger.Error($"调用IM_api报错：url:{url},参数：{parameterobj},异常：{exception.Message}");
                return null;
            }
            finally
            {
        ...
        public static string DeleteResponseForIM(string url, out bool isSuccess, string header_token = null, bool isRetry = false)
        {
            HttpClient httpClient = new HttpClient(new HttpClientHandler() { AutomaticDecompression = DecompressionMethods.GZip });
            HttpResponseMessage response = null;
            isSuccess = false;
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(header_token))
                    httpClient.DefaultRequestHeaders.Add("Authorization", $"Bearer {header_token}");
                Task<HttpResponseMessage> taskResponse = httpClient.DeleteAsync(url);
                taskResponse.Wait();
                response = taskResponse.Result;
                Task<System.IO.Stream> taskStream = response.Content.ReadAsStreamAsync();
                taskStream.Wait();
        ...
        public static string PutResponseForIM(string url, object parameterobj, out bool isSuccess, string header_token = null, bool isRetry = false)
        {
            HttpClient httpClient = new HttpClient(new HttpClientHandler() { AutomaticDecompression = DecompressionMethods.GZip });
            HttpResponseMessage response = null;
            string result = "";
