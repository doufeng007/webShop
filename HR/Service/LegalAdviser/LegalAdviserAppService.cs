...
namespace HR
{
    public class LegalAdviserAppService : FRMSCoreAppServiceBase, ILegalAdviserAppService
    {
        private readonly IRepository<LegalAdviser, Guid> _repository;
        ...
        /// <returns></returns>
        public async Task<PagedResultDto<LegalAdviserListOutputDto>> GetList(GetLegalAdviserListInput input)
        {
            var query = from a in _repository.GetAll().Where(x => !x.IsDeleted)
                        join b in _userRepository.GetAll() on a.UserId equals b.Id
                        select new LegalAdviserListOutputDto()
                        {
                            Id = a.Id,
                            UserId = a.UserId,
                            Name = a.Name,
            ...
            return new PagedResultDto<LegalAdviserListOutputDto>(toalCount, ret);
        }
        ...
        /// <returns></returns>
        public async Task<LegalAdviserOutputDto> Get(EntityDto<Guid> input)
        {
            ...
            var ret = model.MapTo<LegalAdviserOutputDto>();
            ...
            ret.FileList = await _abpFileRelationAppService.GetListAsync(new GetAbpFilesInput() { BusinessId = input.Id.ToString(), BusinessType = (int)AbpFileBusinessType.人力资源法律顾问合同 });
            return ret;
        }
        /// <summary>
        /// 添加一个LegalAdviser
        ...
        /// <returns></returns>
        public async Task Create(CreateLegalAdviserInput input)
        {
            ...
            var newmodel = new LegalAdviser()
            {
                UserId = userId,
                Name = input.Name,
                Function = input.Function,
                ScaleNum = input.ScaleNum,
