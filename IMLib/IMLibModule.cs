...
namespace IMLib
{
   ...
   typeof(ZCYX.FRMSCore.FRMSCoreApplicationModule),
    ...
    public class IMLibModule : AbpModule
    {
...
注意：当给内嵌的XML文件起名字时，要加上语言后缀，但是不要使用“.”，比如“MySource.ch.xml”，而要使用短号“-”，比如“MySource-en.xml”。因为当寻找资源时，“.”会造成问题。
            ...
            Configuration.Authorization.Providers.Add<IMLibAuthorizationProvider>();

            //配置导航菜单
            Configuration.Navigation.Providers.Add<IMLibNavigationProvider>();

        }
        public override void Initialize()
        {
            IocManager.RegisterAssemblyByConvention(typeof(IMLibModule).GetAssembly());
